...
class TWINModelSharedEmb(CTRBaseModel):
    """TWIN with shared item/cate embedding (Step 16).

    cate_emb reuses item_emb's weight table — both share the same
    embedding matrix, saving 80 × 32 = 2,560 parameters.
    """

    def __init__(self, cfg: Config, field_dims: FieldDims):
        super().__init__(cfg, field_dims)
        # Remove the standalone cate_emb from the module tree, then alias to item_emb.
        delattr(self, "cate_emb")
        self.cate_emb = self.item_emb  # alias, not a new module
