...
class TWINModelNonLinearSim(CTRBaseModel):
    """TWIN with non-linear similarity refinement (Step 2).

    Compared to vanilla TWIN, this version adds a small MLP after the raw
    dot-product similarity to learn a non-linear transformation of the
    attention scores.  This helps the model capture complex query-key
    interactions that a pure dot-product cannot express.

    Key change in ``cp_gsu``:
        raw_scores = einsum("bhd,bshd->bsh", query, keys) / sqrt(d)
        attn_scores = sim_mlp(raw_scores.unsqueeze(-1)).squeeze(-1)

    Params added: sim_mlp (65 params, <0.2% of total).
    """
    ...
    def __init__(self, cfg: Config, field_dims: FieldDims):
        ...
        # Non-linear similarity refinement MLP.
        # 1→16→1 with heavy dropout + residual connection to preserve
        # raw dot-product scores as the primary signal.
        ...
        # Per-head compression of hand-built cross features into a single bias.
    ...
    def cp_gsu(
        ...
        # --- Non-linear similarity refinement (Step 2) ---
        raw_scores = torch.einsum("bhd,bshd->bsh", query, keys) / math.sqrt(self.head_dim)
        attn_scores = self.sim_mlp(raw_scores.unsqueeze(-1)).squeeze(-1)
        # --- End Step 2 ---
