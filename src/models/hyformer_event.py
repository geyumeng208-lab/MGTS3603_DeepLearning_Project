...
class EventAwareHyFormerModel(CTRBaseModel):
    """HyFormer with temporal features and behavior-type embeddings."""
    ...
    def __init__(self, cfg: Config, field_dims: FieldDims):
        ...
        non_seq_dim = cfg.embedding_dim * 6 + 4
    ...
    def forward(self, batch: dict[str, torch.Tensor]) -> torch.Tensor:
        ...
        hist_item = (hist_item + event + temporal) * mask.unsqueeze(-1).float()
        hist_cate = (hist_cate + event + temporal) * mask.unsqueeze(-1).float()
        ...
        event_mean = masked_mean(event, mask)
        ...
        non_seq_x = torch.cat(
            [user, target_item, target_cate, item_mean, cate_mean, event_mean, time_summary],
            dim=-1,
        )
