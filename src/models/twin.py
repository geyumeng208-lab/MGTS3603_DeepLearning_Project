...
class TWINModel(CTRBaseModel):
    """Paper-inspired TWIN with CP-GSU and multi-head target attention.

    This is still a compact course-project implementation, but it follows the
    key idea from TWIN more closely than the previous lite version:

    - GSU and ESU share the same target-attention relevance function.
    - Behavior inherent features are projected into multi-head keys/values.
    - Target-behavior cross features are compressed into a scalar bias per head.
    - Top-K retrieval and final interest aggregation reuse the same scores.
    """
