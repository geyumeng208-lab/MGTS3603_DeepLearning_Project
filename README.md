...
本项目的 HyFormer 系列模型基于 [WestbrookLong/Hyformer_Pytorch](https://github.com/WestbrookLong/Hyformer_Pytorch) 的 `HyFormerBackbone`。由于 `external/` 属于外部源码目录，已经被 `.gitignore` 排除，clone 本仓库后需要手动下载并放回项目根目录。
...
```bash
mkdir external
git clone https://github.com/WestbrookLong/Hyformer_Pytorch.git external/Hyformer_Pytorch
```

如果网络不方便，也可以直接下载该 GitHub 仓库的 ZIP，解压后重命名为：
...
```text
external/Hyformer_Pytorch/main_pytorch.py
```

我们自己的 HyFormer 模型会从该文件中导入：

```python
from main_pytorch import HyFormerBackbone, ensure_non_empty_mask, masked_mean_pool
```

因此，如果运行 HyFormer 相关模型时报错：

```text
ModuleNotFoundError: No module named 'main_pytorch'
```

通常说明 `external/Hyformer_Pytorch/` 没有放到项目根目录，或者文件夹名字不一致。请按上面的目录结构重新放置。

`external/Hyformer_Pytorch/AGENTS.md` 是外部仓库自带的开发说明文件，主要给 AI coding agent 或自动化开发工具阅读，描述仓库结构、核心类和修改注意事项；它不会参与训练，也不会被 Python 自动执行。真正被本项目导入的是 `main_pytorch.py` 中的 HyFormer backbone。

安装好外部依赖和 Python 包后，可以先运行 smoke test 检查环境：

```bash
python -m unittest discover tests -p "*_test.py"
```

如果该测试通过，说明数据切分、基础解析逻辑，以及 LSTM、SIM、TWIN 和 HyFormer 系列模型的基础前向传播都可以正常运行。
...
注意：运行 `hyformer*` 相关模型前，需要先完成上面的 `external/Hyformer_Pytorch/` 下载和放置；如果只运行 `base`、`sim`、`eta`、`twin`，则不依赖外部 HyFormer backbone。
...
为了进一步提高代码审查和课程汇报的完整性，后续优化实验按以下方向分工。所有成员统一记录 `AUC`、`GAUC`、`latency`、训练 epoch、数据文件、`batch_size` 和 `max_seq_len`，确保实验可以横向比较。

| 负责人 | 优化方向 | 主要任务 | 输出内容 |
| --- | --- | --- | --- |
| 成员 A | 蒸馏实验 | 使用 `HyFormer-Hierarchical` 作为 teacher，`HyFormer-Static` 或 `HyFormer-OfflineLong` 作为 student，对比蒸馏前后效果 | Teacher-Student 框架图、AUC/GAUC/latency 对比表 |
| 成员 B | 模型压缩实验 | 对比 `hyformer_layers=1`、`embedding_dim=16`、`hyformer_ff_dim=64`、`long_num_chunks=4` 等轻量化设置 | 参数量、latency、AUC/GAUC trade-off |
| 成员 C | TWIN 优化实验 | 调整 `top_k` 和 `twin_heads`，并尝试时间衰减或行为类型 embedding | TWIN 原版、TWIN 优化版与 HyFormer 的效果和时延对比 |
| 成员 D | 长序列与部署优化 | 对比 `HyFormer-Hierarchical`、`HyFormer-Dynamic`、`HyFormer-TopK`、`HyFormer-OfflineLong`，并测试 `recent_seq_len=50/100/200` | 长序列利用方式对效果和 latency 的影响，最终工业部署方案 |

常用命令示例：

```bash
python train.py --model hyformer_hier --data_path data/purchase_sequence_100k_static_long500.csv --epochs 3 --batch_size 128 --max_seq_len 500
python train.py --model hyformer_hier --data_path data/purchase_sequence_100k_static_long500.csv --epochs 1 --max_seq_len 500 --hyformer_layers 1
python train.py --model hyformer_hier --data_path data/purchase_sequence_100k_static_long500.csv --epochs 1 --max_seq_len 500 --embedding_dim 16 --hyformer_ff_dim 64
python train.py --model hyformer_hier --data_path data/purchase_sequence_100k_static_long500.csv --epochs 1 --max_seq_len 500 --long_num_chunks 4
python train.py --model twin --data_path data/purchase_sequence_100k_static.csv --epochs 1 --max_seq_len 100 --top_k 20 --twin_heads 2
python train.py --model hyformer_dynamic --data_path data/purchase_sequence_100k_static_long500.csv --epochs 1 --max_seq_len 500 --dynamic_recent_len 50
python train.py --model hyformer_hier --data_path data/purchase_sequence_100k_static_long500.csv --epochs 1 --max_seq_len 500 --recent_seq_len 200
```
